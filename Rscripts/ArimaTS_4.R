...
con <- read.csv(file = "TimeSeries11.csv", header = T, sep = ",")
...
fit_no_holdout = arima(ts(deseasonal_cnt[-c(126:168)]), order=c(1,1,8))
...
fit4 = arima(deseasonal_cnt, order=c(1,1,8)) 
...
accuracy(fit3) #auto arima (3,0,0) fit without seasonality
accuracy(fit4) #custom arima (1,1,7) fit
